...
local uiScreen = Instance.new('ScreenGui') do 
    ...
    -- =======================================================
    -- ХОТКЕЙ INSERT ДЛЯ СКРЫТИЯ МЕНЮ
    -- =======================================================
    local menuVisible = true
    inputService.InputBegan:Connect(function(input, gameProcessed)
        -- Не проверяем gameProcessed, чтобы меню скрывалось, даже если открыт чат
        if input.KeyCode == Enum.KeyCode.Insert then
            menuVisible = not menuVisible
            uiScreen.Enabled = menuVisible -- Переключаем видимость главного ScreenGui
        end
    end)
    -- =======================================================  
...
do 
    ...
    do
        ...
        -- =======================================================
        -- КАСТОМНЫЙ АНИМЕ ФОН ДЛЯ ОКНА
        -- =======================================================
        local animeBackground = Instance.new("ImageLabel")
        animeBackground.Name = "CustomAnimeBg"
        -- Растягиваем на всё окно, но сверху отступаем 26 пикселей под шапку меню
        animeBackground.Size = UDim2.new(1, 0, 1, -26) 
        animeBackground.Position = UDim2.new(0, 0, 0, 26) 
        animeBackground.Image = "rbxassetid://14241031174" -- ID арта с тянкой
        animeBackground.ImageTransparency = 0.83 -- Прозрачность (0.83 — чтобы текст читался идеально)
        animeBackground.BackgroundTransparency = 1
        animeBackground.ScaleType = Enum.ScaleType.Crop
        -- Ставим ZIndex = 6, чтобы картинка была чуть выше самого фрейма (ZIndex 5), 
        -- но ниже всех кнопок и текста, у которых ZIndex в Dollarware идет дальше
        animeBackground.ZIndex = 6 
        animeBackground.Parent = mainFrame

        -- Проверяем скругление углов, если в этой теме используется rounding
        if rounding then
            local bgCorner = Instance.new("UICorner")
            bgCorner.CornerRadius = UDim.new(0, 4)
            bgCorner.Parent = animeBackground
        end
        -- =======================================================
...
local backgroundFrame = Instance.new('Frame') do 
                    ...
                    -- НАСТРОЙКА КАРТИНКИ:
                    backgroundImage.Image = 'rbxassetid://403444734' -- Сюда потом вставишь ID своей тян
                    backgroundImage.ImageTransparency = 0.4 -- Прозрачность (0.4 — оптимально)
                    backgroundImage.ScaleType = Enum.ScaleType.Crop
                    backgroundImage.ZIndex = 4
            ...
            window.setBackground = function(self, image, transparency) 
                local bg = self.instances.backgroundImage
                if (bg) then
                    bg.Image = (typeof(image) == 'number' and 'rbxassetid://' .. tostring(image)) or tostring(image or '')
            ...
            window.new = function(self, resize) 
                ...
                instances.backgroundImage = instances.mainFrame:FindFirstChild('AnimeBackground', true)
        ...
        local pickerWindow = {} do 
            ...
            local instances = {} do 
                ...
                local backgroundFrame = Instance.new('Frame') do 
                    ...
                    -- Добавляем фон и в окно выбора цвета для стиля
                    local backgroundImage = Instance.new('ImageLabel')
                    backgroundImage.Name = 'AnimeBackground'
                    backgroundImage.AnchorPoint = Vector2.new(0.5, 0.5)
                    backgroundImage.Position = UDim2.fromScale(0.5, 0.5)
                    ...
                    backgroundImage.Image = 'rbxassetid://403444734'
                    backgroundImage.ImageTransparency = 0.5
                    backgroundImage.ScaleType = Enum.ScaleType.Crop
                    backgroundImage.ZIndex = 99
...
do
    ...
    ui.newWindow = function(settings) 
        ...
        local s_background = settings.background
        local s_bgTransparency = settings.backgroundTransparency

        if (s_icon) then
            window:setIcon(s_icon) 
        end
        if (s_background) then
            window:setBackground(s_background, s_bgTransparency)
        end
